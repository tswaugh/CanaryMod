...
/**
 * This is a connection to the canarymod database.
 * Use it for your database management stuff.
 * @author Chris
 *
 */
public class CanaryConnection {
	...
	public boolean isLeased() throws SQLException {
		if(leased && !con.isClosed()) {
			return true;
		}
		else if((!con.isClosed()) && !leased) {
			return false;
		}
		return true;
	}
	...
	/**
	 * Returns the actual connection and sets the CanaryConnection to leased,
	 * so it will not be used by something else
	 * @return
	 */
	...
	/**
	 * Sign this connection as unleased. (Not used anymore).
	 * Connection will remain idling until it is used again or
	 * removed by the connection guard
	 */
	public void release() {
		leased = false;
	}
